...
| `cite_check.rs::AUTHORITY_NOT_YET_ARCHIVED` (shrink-only, `(form, years)`, **31 of 36** pairs excused as of 2026-09-06 — was 36 of 37 when this was written; S9 dropped the five TY2017 pairs and residue sweep 1 archived + extracted the four Form 4868 / Form 1040-V pairs) | ★ **RECONCILED 2026-09-13 (A3/FR-138 landed): there is now ONE notion, the manifest join.** `AUTHORITY_NOT_YET_ARCHIVED` is DELETED; `archived_form_years()` joins each map row (`irs_stem` + `instructions` + year) to `design/forms/MANIFEST.json`, requiring for the form AND its instructions an entry that is IRS-final not draft, records the conventional `design/forms/extract/<stem>--<year>.txt`, and is present on disk — and the excuse set is read off each row's own `authority = "not-yet-archived: <reason>"` header. Measured after integration: **37 of 38 archived, 1 excused (`f8283--2024`), 0 unaccounted** (was 7/38). ★★ And **30 of the 31 excuses this cell counted were FALSE** — the old join could not see documents that were archived, extracted, hashed and manifested. Superseded text follows. This was a DIFFERENT "archived" from the manifest join: it means "no `FormAuthority` row + extract for cite-check", and it retires as map headers gain `instructions`/extract coverage. The MANIFEST join (`template_sha256`) is the other notion and reds today on exactly **6 of 37** templates — all five TY2017 and `forms/2024/f8283.pdf` (measured by sha256 join, fold review F7) — so the header gets `authority = "not-yet-archived: <reason>"` for those six, and the join kill treats that field as the excuse. Two of the six ride on the open TY2017 decision. **★ That note is DISCHARGED — A3 landed 2026-09-13; see the reconciliation at the head of this cell.** |
| `BundledFullReturnTables` 2024-only | **untouched** — it is the compute gate; `YEAR.toml` `status` declares, it decides |

**★ DISCHARGED 2026-09-13 — A3 LANDED, and S4's read was right: the two notions did become one.** The reconciliation is at the head of the `AUTHORITY_NOT_YET_ARCHIVED` row above; this note is kept because it records what was and was not verifiable from an isolated worktree. Original note: Agent A3, working
...
**★ DISCHARGED 2026-09-13 — A2 LANDED, and S4's FIRST CANDIDATE WAS CORRECT.** §10 step 3's *"`LineSet` (37) / `Schema` (17 + `Unwired`) / the exhaustive `schema()` match"* did describe `LineSet` as a hand-maintained enum, and it is now generated by `build.rs` from each map's own `line_set` row — `line_set.rs` keeps only `Schema` and an `_`-free `const fn schema()`. Per-port cost in those three files went **7 → 1**. This note is kept because it records a correct inference drawn WITHOUT the diff. Original note: Agent A2, working
