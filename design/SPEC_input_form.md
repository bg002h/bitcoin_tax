...
3. **Attribution — `btctax-input-form` — an EXHAUSTIVE `match RefuseReason -> Vec<FieldId | SectionId>`.**
   ...
   **The v1 attribution map** (the exhaustive `match`; input-screenable reasons a v1 form can surface):
   ...
   | everything else (`BusinessInterestIncome`, `BusinessIncomeWithoutScheduleC`, `ScheduleCLoss`, `ScheduleCNoBusinessDescription`, `KiddieTax`, `QbiAboveThreshold`, `AmtScreenTriggered`, `ForeignTaxOverCeiling`, `IraDeductionClaimed`, `PrivateActivityBondAmt`, `UnrecapturedOrSpecialRateGain`, `InconsistentDividendSubset`) | `NotInForm { note }` — a **deferred section** (Schedule C, QBI, 1099s incl. `PrivateActivityBondAmt`'s 1099-INT box 9 / 1099-DIV box 13 — I-3, carryforwards) or a **compute/absolute** screen; the form says "entered via TOML import / computed at `report`" |
