...
- **★ Session access (review I-1): there is NO `session()`/`session_mut()` method** — `session` is a bare field `pub session: Option<Session>` (`editor.rs:89`). Read via `app.session.as_ref().unwrap().conn()`; get `&mut Session` via `app.session.as_mut().unwrap()` (the `handle_modal_key` idiom, `main.rs:472`). **Borrow `app.session` and `app.tax_inputs_form` as DISJOINT fields** — do NOT route a `&mut Session` through a whole-`self` accessor: the autosave/commit paths hold a live `app.tax_inputs_form.as_mut()` borrow, and a `&mut self` method would conflict and not compile. `app.session.as_mut()` borrows only the `session` field (disjoint), so it compiles.
- **★ Bundled tables (review M2): bind the bundle to a `let` before looking up** — `table_for`/`full_return_for` borrow `&self`, so `BundledTaxTables::load().table_for(year)` is a borrow-of-dropped-temporary. Do `let tables = BundledTaxTables::load(); let table = tables.table_for(year);` (mirror `answer.rs:309-310`), and bring the trait methods into scope: `use btctax_core::TaxTables;` + `use btctax_core::tax::tables::FullReturnTables;`.
...
- [ ] **Step 1: Add the dep, wire the field + a minimal opener + a render stub, and write a key-driven test that opening on a fresh year yields a `None` working copy.** First (RED needs it to compile): add the Cargo dep; add `pub tax_inputs_form: Option<TaxInputsFormState>` to `EditorApp` (init `None` in `EditorApp::new`); add the `TaxInputsFormState` struct (minimal fields); add a `handle_key` dispatch early-return `if app.tax_inputs_form.is_some() { handle_tax_inputs_key(app, key); return; }` placed with the other flow gates (before the Browse `match`, after modals); bind the FREE Browse key `KeyCode::Char('T')` (confirmed unused in the Browse `match key.code` at `main.rs:397-452`) to `open_tax_inputs_form(app)` — **and add `T` to the help/keymap overlay (`draw_help_overlay`, the "KEEP IN SYNC" note at `main.rs:449` — review M4)**; add a stub `handle_tax_inputs_key` (Esc closes the flow) and a stub `draw_tax_inputs_form`. **Define a local `flatten(buffer) -> String` test helper (review M3) — the crate has no named one; inline the `draw_edit.rs:5278-5285` cell-collect pattern into a small fn.** **The `unlocked_app_on_empty_vault(2024)` helper MUST set `app.selected_year = 2024` explicitly (review M9)** — an empty vault defaults `selected_year` to 2025 (`editor.rs:95`), and 2025 has no full-return params (a stray commit would hit `NoTables`).
...
- [ ] **Step 3: Implement `open_tax_inputs_form`** — reuse `app.selected_year`. Call `input_form_store::load(app.session.as_ref().unwrap().conn(), year)` (the disjoint-field idiom — review I-1):
...
- [ ] **Step 2: Run fail. Step 3: implement `draw_tax_inputs_form`.** Split `area` into left (section list), right (field pane), bottom (status) with a `Layout` (mirror the profile-form `Layout`/`centered_rect` usage). **★ First branch (review M7): if `form.discard_offered` (the P2-a stale-parked state from Task 1), render ONLY the stale-parked message + "press X to discard the parked draft, Esc to back out" — do NOT render the editing form.** Otherwise: when `working` is `None`, show ONLY `ReturnOptions`'s `FilingStatus` field (NI-2). **Live sections (when `Some(ri)`):** iterate `form_spec()`, keep those whose `Section` is live: a `Singleton` is always shown; a `Repeating` always shows (with its rows); an `OptionalSingleton` per the rules below. **★ Section-visibility rules (review I-2 — these are TUI-side knowledge the engine's `OptionalSingleton` does not carry):**
  - **`ScheduleA`**: always OFFERABLE — shown; `present(ri)==false` renders a `[create]` affordance, `true` renders its fields.
  - **`Spouse`**: offered IFF `ri.filing_status ∈ {Mfj, Mfs, Qss}` (read via the `FilingStatus` `get` accessor, not the struct field). On `Single`/`HoH` the Spouse section is ENTIRELY HIDDEN. When offered: `present(ri)==false` → shown with `[create]`; `present(ri)==true` → its fields. (So three visual states: Single/HoH → absent; MFJ/MFS/QSS + no spouse → `[create]`; + spouse → fields. `present(ri)` alone can't distinguish fresh-Single from fresh-MFJ, hence the filing-status gate.)
  - **`W2Box12`** and **`ScheduleACharitable`** are top-level `form_spec()` entries but are LOGICALLY NESTED (box12 under a W-2 row, charitable under Schedule A). Do NOT render them as their own top-level left-pane sections — they appear as nested rows within their parent's row-editing (Task 5 owns box12 at depth 2 and charitable rows). In this task, simply SKIP `W2Box12`/`ScheduleACharitable` in the left-pane section list.
  Compute per-section status glyph: `✓` all live fields set / `…` incomplete / `!` a screen refusal attributes here (Task 7 wires `!`; here `✓`/`…`). **Field pane:** for the selected section, iterate its live `Field`s (`field.live(ri)`), render `label  [value]  ‹error›` where value = the `get` accessor's `FieldValue` rendered per kind (Money → `$x`, TriState → `yes/no/—`, Enum → the choice, Secret → the `SecretView` masked/`(unset)`, Date → `YYYY-MM-DD`/`—`), focus-styled like the profile form. **Status line:** `active source: full return|tax-profile` (Task 8) + the screen status + key legend; if `form.stale_note.is_some()`, show its `Display` (e.g. "discarded a stale draft…"). Navigation in `handle_tax_inputs_key`: `Up/Down` move field focus within the section; `Left/Right`/`Tab` move `section_idx` across live sections (clamp); recompute live sections each keypress (they change after edits).
...
**Interfaces:** Consumes `input_form_store::save_draft`. Produces a debounced autosave: after a mutating edit, mark the flow dirty; flush `save_draft(sess, year, ri)` on section change and on a short idle tick (NOT per keystroke). Only a `Some(working)` writes. **★ Also owns the `q` key (review I-3):** `q` inside the flow warns if `dirty`, flushes the draft, then closes the flow (the draft is already autosaved, so quit is safe — §9A). Without this, the flow gate silently swallows `q`.
...
- [ ] **Step 2: fail. Step 3: implement.** Add `dirty: bool` to `TaxInputsFormState`; set it on every successful mutating `apply`. Flush points: on `Left/Right`/`Tab` section change, and on the editor's idle/tick path if one exists (grep `editor.rs` for a tick/poll timeout; if the crate polls with a timeout, flush on timeout when `dirty`; else flush on section change + on flow close). Flush = `if form.dirty { if let Some(ri) = form.working.clone() { save_draft(app.session.as_mut().unwrap(), year, &ri)?; app.tax_inputs_form.as_mut().unwrap().dirty = false; } }` — note the disjoint-field access (review I-1): clone the `ri` out (or scope the `form` borrow) so `app.session.as_mut()` and `app.tax_inputs_form` don't alias. `save_draft` preserves `parked` (NI-1, plan 2). **Error routing (review M1):** `save_draft`/`commit`/`park`/`discard` return `CliError`, NOT `edit::persist::PersistError` — do NOT route them through `on_persist_error` (which takes `PersistError`); instead set `app.status = Some(format!("{e}"))` directly (or add a small `CliError`→status helper). **Do NOT autosave a `None` working copy** (nothing chosen yet).
...
- [ ] **Step 2: fail. Step 3: implement.** `s` requires `working.is_some()` (else "choose a filing status first"). Build the confirm modal as a NESTED field on the flow state — `form.modal: Option<TaxInputsModalState>` (review M5: nested in `TaxInputsFormState`, dispatched inside `handle_tax_inputs_key` — checked before the field keymap — NOT a separate top-level `EditorApp` `Option`). `TaxInputsModalState { year, filing_status_label: String, summary: String, shadows: bool }` — **the `filing_status_label` and summary read the filing status via the `FilingStatus` `get` accessor, NEVER `ri.filing_status` (review M6 — Task 9 tests "never names a `ReturnInputs` field")**; the summary names the filing status (I-9), the sections present (n W-2s, Schedule A?, n dependents), and if `shadows_profile(conn, year)` the shadow + all-zero warning ("your tax-profile estimate stays saved and unused"; a declarations-only return commits ≈ $0). Modal Enter (with the disjoint-field + bound-bundle idioms — reviews I-1/M2): `let tables = BundledTaxTables::load(); let fr = BundledFullReturnTables::load(); let (table, params) = (tables.table_for(year), fr.full_return_for(year)); let out = commit(app.session.as_mut().unwrap(), year, &ri, table, params)?;` (with `use btctax_core::TaxTables; use btctax_core::tax::tables::FullReturnTables;`) then match:
