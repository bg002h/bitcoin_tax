...
- `compute_tax_year(events: &[LedgerEvent], state: &LedgerState, year: i32, profile: Option<&TaxProfile>, tables: &dyn TaxTables) -> TaxOutcome` — `LedgerEvent` (`event.rs:263-271`), `LedgerState` (`state.rs:176-186`); reads `state.disposals` (`:180`) → `Disposal.{disposed_at,legs}` (`:111,112`) → `DisposalLeg.{gain,term}` (`:101,102`); `state.income_recognized` (`:182`) → `IncomeRecord.{recognized_at,usd_fmv}` (`:143,145`); `state.blockers` (`:184`) → `Blocker.{kind,event,detail}` (`:65-70`) + `BlockerKind::severity()` (`:47-64`) + `Severity::Hard` (`:17-21`); `EventId::canonical()` (identity.rs, used by render/eventref today). All money is `Usd = Decimal` (`conventions.rs:8`); `round_cents` (`conventions.rs:22`). **Note (B-R2-N1):** `tax_date(utc,tz)` was removed from this list — after the B-I1 fold the function filters on `disposed_at.year()` / `recognized_at.year()` and no longer calls `tax_date()`; it remains correctly cited in the §1 grounding table above.
