...
- **`design/TY2026_WORK_LIST.md`** — every form's delta, computed. `f1040` and `f6251` rename
  ...
  fields survive), corroborating R2. ★ The list omits two emitted forms (`f1040s1`, `f8283`) because
  it enumerates from available fixture PAIRS, not from the emitting surface — Fable plan review I8,
  now stated per cell in the list and filed as a `forms port-status` instrument.
