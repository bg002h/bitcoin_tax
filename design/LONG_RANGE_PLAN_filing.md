...
> | **4** — the last mile | must precede FILED | ◐ **OPEN, and the smallest open phase — but NOT for the reason first written here.** ★ **CORRECTED 2026-09-11 (same day):** an earlier revision of this banner listed **FR-49** as remaining. **FR-49 is CLOSED** (2026-09-06, spec GREEN r6): `btctax extension` (`cli.rs:271`) and `export-irs-pdf --pay-by-check` (`cli.rs:244`) exist, the §7503 weekend AND District-of-Columbia-holiday shifter landed, and both residue items closed. Also already done: the spouse IP PIN (T10, `return_inputs.rs:1122`) and the attachment-order/staple/envelope guidance (`hand_marks`, `cmd/admin.rs`). **What actually remained was two items and a rehearsal — and BOTH ITEMS ARE NOW CLOSED (2026-09-11, `design/agent-reports/REPORT-build-phase4-last-mile.md`; the exit gate is MET, leaving only (iii), which is not buildable).** (i) ✅ **WHERE TO FILE — BUILT.** The packet manifest gained a `WHERE TO POST IT` block (`cmd/admin.rs` `where_to_post_block`) and `btctax extension` its own (`render.rs` `render_extension`), both carrying the two facts from one pair of constants (`btctax-cli/src/lib.rs` `WHERE_TO_FILE_STATE_FACT` / `WHERE_TO_FILE_PAYMENT_FACT`) plus the year's-table pointer; `LIMITATIONS.md` carries the same. The no-table decision is now held STRUCTURALLY by `crates/xtask/src/service_center_check.rs`, which reds if a postal address enters shipped text, and whose complement asserts the tables btctax points AT really exist in every bundled year. Original text of the finding: (i) ★ **WHERE TO FILE** — measured: zero `irs.gov` where-to-file or service-center references anywhere in `crates/`, so a filer holding a finished packet does not know where to post it, and **the phase's own exit gate is therefore unmet** (*"a filer holding the packet can post it without consulting anything outside it"*). The plan's verdict stands: a **link plus the two facts** (state-dependent AND payment-dependent), never a bundled address table that rots. (ii) **record retention** — the design decision exists (`design/forms/FIELD_PROVENANCE.md:265,:464` — never auto-shred; the window is the filer's call) but it is **not filer-facing**: no mention in `LIMITATIONS.md` or the manifest, which is where the plan said to document it. ✅ **CLOSED 2026-09-11**: a `KEEPING YOUR RECORDS` block in the manifest and a section in `LIMITATIONS.md`, both from one constant (`RECORD_RETENTION_GUIDANCE`), quoting the instruction's *"as long as they are needed to figure the basis"* sentence and naming why the generic three years is the wrong instinct for a crypto ledger — with **no date, no deadline, no default and no retention mechanism**, held by a test that reds on a year, an ISO date or a month name in the block. This is how the two sources were reconciled: the plan's *holding period + 3 years* is the MECHANISM, stated; FIELD_PROVENANCE:464's *"do not pick a window"* is the OUTPUT, respected. (iii) **S8's physical print rehearsal** — the owner's to perform, not buildable. ★ And its original ★ item is **CLOSED**: `broker_reported_rows` is no longer hardcoded to `0` — it is a real field (`cmd/admin.rs:345`) fed from `main.rs:1092` |
