...
**SelfTransfer scope note (A-M1):** A `SelfTransfer` is listed as method-honoring in A.3 (the lot-routing choice pre-positions lots for future disposals), but it is a *non-taxable positioning move* — it produces no Disposal or Removal record, carries no §1.1012-1(j) identification obligation, and is **intentionally out of scope for `disposal_compliance`**, which covers only taxable dispositions.
