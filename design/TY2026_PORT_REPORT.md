...
| **21** | `crates/btctax-core/src/tax/printed.rs` as a whole — 129 line-numbered fields across 10 structs (measured 2026-09-05; the first edition said 107 across 7), **zero references to `year` in non-test code** | **FALLS BACK** | the printed layer's line numbering is decided by nothing a year can change. This is the structural root of #19 and #20 (compute B2 row 26) |
...
|---|---|---|---|
| **quantity** — *which number this is* | semantic field name, line number in the doc comment — **for a CROSS-YEAR quantity**. A per-revision transcription struct is line-named BY RULE (`CLAUDE.md`, scope amended 2026-09-05): `schedule_1a.rs` (56) was built under it and reviewed green, and the first edition's listing of it as the bad example was a doctrine reversal presented as synthesis (Fable plan review I2), withdrawn here, at the site | `AbsoluteReturn` (`return_1040.rs:1530-1660`) — zero `lineNN` fields | **OPEN:** `printed.rs` (`Form1040Lines` 34, `ScheduleALines` 22, `ScheduleDLines` 18 `lineNN` fields; **129** across 10 structs, none carrying a `year`, serving 2017/2024/2025 alike) and `qbi.rs::Form8995Lines` (16) — line-named structs shared across three revisions with no `line_set`; whether each is a transcription struct that needs a revision or a quantity struct that needs semantic names is decided when `line_set` lands (design r2 §7). Not blessed by the withdrawal above (fold review r2 G5) |
...
| **new** | **`printed.rs` has no `year` in scope at all** | — | **UNGUARDED / IMPORTANT** | 129 line-numbered fields across 10 structs (measured 2026-09-05; the first edition said 107 across 7) whose numbering is decided by nothing a year can change — the structural root of the two rows above |
...
| **R25** | a year bump that does not extend `TAXCALC_EXACT_YEARS` makes taxcalc smooth the Schedule 1-A step (±$100/$200) | LIVE / IMPORTANT | **CLOSED 2026-09-12 (FR-124)** | ★ The list is gone, not extended. `TAXCALC_EXACT_YEARS` was ALSO inert — `exact` is a calculated taxcalc variable, so the Records input column it was written as had been silently dropped since it landed, and TY2025 was already taking the smooth branch. `scripts/oracle/taxcalc_exact.py` is now the one construction path: it writes `exact` through the Calculator, asserts it stuck after `calc_all`, refuses a row dict carrying the key, and **reverses the polarity** — ON for every year except the baked-corpus years in `EXACT_OFF_YEARS = {2024}`, so a year bump gets the form-faithful branch by doing nothing. Kill: `.venv/bin/python scripts/oracle/taxcalc_exact.py --selftest` (44 fractional-step vectors move off the smooth fallback) |
