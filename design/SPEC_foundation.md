# SPEC — bitcoin_tax (TaxApp), Phase 1: Foundation (v0.4)

- **Status:** DRAFT v0.4 — folds round-1/2/3 independent reviews (tax + engineering + architecture). Pending round-4 re-review to green (0 Critical / 0 Important) before the plan gate.
- **History:** v0.1 → v0.2 (R1 fold) → v0.3 (R2 fold) → v0.4 (R3 fold). Superseded specs in git history; all reviews in `reviews/`.
- **Date:** 2026-06-28
...
- **FR8 Corrections.** `VoidDecisionEvent` revokes a *revocable* decision. An **effective `SafeHarborAllocation` is irrevocable** (an inert/time-barred one is not yet effective). `ImportConflict`s resolve only via `SupersedeImport`/`RejectImport`.
...
- **`EventId`** (universal reference target):
  ...
  - **`ImportConflict` (system-generated):** `f("conflict", source, source_ref, new_fingerprint)` — **distinct from its target's `EventId`**; re-importing the identical changed row reproduces the same conflict `EventId` (idempotent); a different change to the same target forms a separate conflict.
...
- **`LotId` = (origin_event_id: EventId, split_sequence: u32)**. `origin_event_id` = the `EventId` of the event that created the underlying holding: `Acquire`/`Income` → that event; `GiftReceived` → the inbound `TransferIn` event (the `ClassifyInbound` decision sets treatment, not identity); **Path-B `SafeHarborAllocation`-seeded lots → the allocation's `EventId`**; Path-A relocates existing lots (origin unchanged). `split_sequence` assigned deterministically as lots split; stable because `EventId` is stable.
...
- `SafeHarborAllocation { lots: [{wallet, sat, usd_basis, acquired_at}], method: ActualPosition|ProRata, effective_date: 2025-01-01, pre_disposition_attested: bool }` — lots with dates; conservation-checked; the `pre_disposition_attested` flag persists the user's attestation that a valid/timely allocation exists in their own books (§7.4); irrevocable once **effective**.
...
### 7.1 Contract
`project(events) -> LedgerState { lots, holdings_by_wallet, disposals, removals, income_recognized, pending_reconciliation, blockers }` — pure, deterministic, no I/O, **total** (never panics). `blockers` = `{ fmv_missing, unclassified, import_conflicts, uncovered_disposal, unmatched_outflows, unknown_basis_inbounds, decision_conflicts, safe_harbor_timebar, pre2025_method_note }`.

### 7.2 Two-pass model (determinism)
- **Pass 1 — resolve decisions onto the imported timeline:** first compute the **non-voided decision set** (a `VoidDecisionEvent` removes its target from the applied set); then apply the remaining decisions **in `decision_seq` order**: `SupersedeImport`/`RejectImport` resolve `ImportConflict`s; `ClassifyRaw` turns `Unclassified`→real payload; `TransferLink`/`ReclassifyOutflow`/`ClassifyInbound` set the **effective treatment** of their target imported events; detect contradictory decisions → `decision_conflicts`. Output: an **effective imported timeline**. (A 2026 decision thus correctly rewrites a 2022 event's effective treatment and folds at the 2022 date.)
...
- `ClassifyInbound` → `Income` lot (FMV basis) or `GiftReceived` lot (gain-basis = donor carryover; `dual_loss_basis` when FMV-at-gift < donor basis; unknown donor basis → FMV-at-`donor_acquired_at` fallback (`GiftFmvFallback`) or a `unknown_basis_inbounds` flag if the date is unknown — TP11).
...
- `ImportConflict` → `import_conflicts` blocker until `SupersedeImport` (replace target payload, same target `EventId`) or `RejectImport` (keep original) resolves it.
...
- `VoidDecisionEvent` → (handled in pass 1) drop a revocable decision; two un-voided conflicting decisions on one target → `decision_conflicts`.
...
- **"Effective" allocation & the time-bar (a *projection* rule, so it re-evaluates deterministically on every rebuild — e.g., when a 2025 disposition is imported later):** a `SafeHarborAllocation` is **effective** iff it passes the guards: **(1) deadline** — specific-unit allocation must precede the earlier of **the first 2025 BTC Sell, Spend, Transfer, Gift, or Donation** (any "sale, disposition, or transfer" per §5.02(4)) or **the 2025 return due date including extension**; the global-method *description* must predate 2025-01-01 (§5.02(5)(a)), with completed allocations due before the later of §5.02(4)(a)/(b) (§5.02(5)(b)). If the effective timeline shows such a first 2025 event **before** the allocation's effective date **and `pre_disposition_attested == false`**, the allocation is **inert** → projection uses **Path A** and raises `safe_harbor_timebar`. If `pre_disposition_attested == true`, Path B governs (the app cannot see the user's own books; the attestation is persisted + auditable). **(2) irrevocable** once effective. **(3) conservation** — Σsat == remaining held sat; Σbasis == remaining pool basis at 2025-01-01. **(4) capital-asset eligibility (§4.02(1)-(2))** assumed for a personal investor; flagged if dealer.
