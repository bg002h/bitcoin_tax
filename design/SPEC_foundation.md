...
- `SafeHarborAllocation { lots: [{wallet, sat, usd_basis, acquired_at, dual_loss_basis?: Option<Decimal>, donor_acquired_at?: Option<Date>}], as_of_date: 2025-01-01, method: ActualPosition|ProRata, timely_allocation_attested: bool }` — lots with dates; conservation-checked. `usd_basis` is the **GAIN basis = donor carryover basis** (§1015(a)); `dual_loss_basis` (optional, `#[serde(default)]`) is the **LOSS basis = FMV-at-gift**, present only when FMV-at-gift < donor basis; `donor_acquired_at` (optional, `#[serde(default)]`) carries §1223(2) tacking for the gain side. Old persisted events without these fields deserialize to `None` (backward-compatible). The allocation's **made-date is the event's `utc_timestamp`** (creation time), distinct from `as_of_date` (the fixed 2025-01-01 snapshot the basis is allocated *as of*). `timely_allocation_attested` persists the user's attestation that they established a valid/timely allocation in their own books **before the §5.02(4) deadline** (covering both the first-2025-disposition and return-due-date prongs, §7.4). Irrevocable once **effective** (§7.4).
...
- **Dual basis + tacking (Slug 1):** Path B preserves a received-gift lot's §1015(a) dual basis (`dual_loss_basis`) and §1223(2) tacking (`donor_acquired_at`) — the allocation carries these fields from the pre-2025 residue into the seeded `Lot`, matching Path A behavior. Conservation checks only `Σsat` and `Σusd_basis` (gain basis); `dual_loss_basis` is an *alternative* basis and is correctly NOT part of the conservation identity.
