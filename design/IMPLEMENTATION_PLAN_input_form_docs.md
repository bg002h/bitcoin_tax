...
**Goal:** Document the new "tax inputs" authoring path (spec §11.9): the `btctax-tui-edit` "tax inputs" mode gets a man-page section, and `LIMITATIONS.md` records that the form is now the primary authoring path for a full-return `ReturnInputs` and what it cannot see at entry — while `income import` (with `income show`/`clear`/`answer`) remains the file-based path.

**★ CORRECTION (plan review, Critical): there is NO `btctax income template` command.** `IncomeCmd` has exactly four variants — `Import`, `Show`, `Clear`, `Answer` (`crates/btctax-cli/src/cli.rs`) — `template` was a planned-but-never-built P8 feature that the SPEC's §11.9 references stale. Do NOT document `income template` anywhere (it would ship a false claim into `LIMITATIONS.md`, which `btctax limitations` prints verbatim, and into the man page — a command clap rejects). Cite only `income import` (and `show`/`clear`/`answer`) as the file-based path.
...
- **`income import` (+ `income show`/`clear`/`answer`) remain** — the form does NOT remove them; it is the primary INTERACTIVE authoring path, complementary to the TOML/file-based commands. Say so; do not imply they are deprecated. **Do NOT write `income template` — it does not exist (see the goal-block correction).**
...
- [ ] **Step 1: Write the failing test first** — in `crates/xtask/src/docs.rs`'s `manpages_have_required_sections`, add `T` to the set of action keys required in `btctax-tui-edit.1` (alongside `?`/`V`/`O`). **★ CORRECTION (plan review, Important): LINE-ANCHOR the match — use `"\n.B T\n"`, NOT `"\n.B T"`.** The page ALREADY contains `"\n.B Tab / Shift\-Tab"` (the Navigation section), which contains `"\n.B T"` as a substring — so an unanchored match passes vacuously today (never red→green) and guards nothing (the tax-inputs section could later be deleted and the KAT would still pass on the `Tab` line). `"\n.B T\n"` (a `.B T` on its own line, `T` followed by newline) does NOT match the `Tab` line and only matches a real `.B T` keymap entry. Run `cargo test -p xtask manpages_have_required_sections` → it FAILS (the man page doesn't yet have a `.B T` line). This is the drift guard: the man page must list the tax-inputs entry key.
...
- [ ] **Step 3: Add the man-page section.** In `docs/man/btctax-tui-edit.1`, add (mirror the existing roff idiom; place after the reconciliation KEY BINDINGS, e.g. a new `.SH "TAX INPUTS MODE"` or a `.SS "Tax inputs (full-return authoring)"`):
  - The entry: `.B T` on the Browse screen opens the "tax inputs" editor for the selected year — the interactive way to author a full-return `ReturnInputs` (header/PII, W-2s incl. box 12, Schedule A incl. charitable, dependents, **Payments (estimated tax / extension / other withholding)**, declarations, skippables) WITHOUT hand-editing TOML. (Render the entry as a `.B T` on its OWN roff line so the Step-1 KAT's `"\n.B T\n"` match is satisfied.)
  ...
  - A pointer: it commits only a `screen_inputs`-clean return; `income import` (with `income show`/`clear`/`answer`) remains the file-based path. (There is NO `income template`.)
...
- [ ] **Step 2: Add the section.** Content (match the file's tone — factual, conservative, fail-closed):
  - **The form is the primary authoring path.** The `btctax-tui-edit` "tax inputs" mode (press `T`) is the primary way to author a full-return `ReturnInputs` — interactive, with live per-field validation, autosave to an encrypted draft, and a commit that writes ONLY a `screen_inputs`-clean return. `income import` (import a filled TOML) — with `income show`/`clear`/`answer` — remains the file-based path; it is complementary, not deprecated. (There is NO `income template` command.)
  - **What the form cannot see at entry.** The commit-screen (`screen_inputs`) is the SAME gate the resolver uses, so a committed return is never worse than what `report` computes — BUT: (a) the v1 form covers only the common subset (header/PII, W-2s incl. box 12, Schedule A incl. charitable, dependents, Payments, declarations, skippables); the **deferred sections** — Schedule C, QBI, 1099-INT/DIV/G, capital-loss / charitable / QBI carryforwards — are NOT authorable in the form yet (enter them via TOML import if needed; the tree already expresses them for a later phase). (b) Some checks fire only at **`report`**, not at the commit-screen — the advisories (e.g. the mixed-use-mortgage allocation, the §164(b)(5) sales-tax election, the §170 qualified-appraisal trigger, blindness/DOB forfeiture prompts) surface when you run `report`, so a form-clean return can still have report-time advisories to act on. The form commits a return that is SCREEN-clean, not necessarily advisory-free.
...
- **Spec coverage (§11.9):** man pages (T1) · `income import` (+ show/clear/answer) remains — NO `income template` (T1 pointer + T2) · `LIMITATIONS.md` form-is-primary + can't-see-at-entry (T2). Fully covered.
