...
/// ★★★ FR-179: `DocumentRow` and [`DocumentRow::ALL`] are GENERATED from the ONE token list passed
/// to this macro, rather than typed twice — an enum body plus a separately-typed `ALL` array that a
/// later edit could silently let drift apart. That drift was exactly the defect: an exhaustive
/// `match` on `DocumentRow` forces ARMS for a new variant, but it does **not** force MEMBERSHIP in a
/// separate array literal, so a 21st variant could gain every arm the compiler demands and still be
/// absent from `ALL` — silently dropping it from every completeness iteration built on `ALL`
/// (`CLAUDE.md`'s "derive the list, or make the compiler hold it"). With the enum and `ALL` sharing
/// one definition, there is no second place left to forget: a variant added below appears in both
/// in the same edit, or the invocation fails to parse. B1 plant: add a variant to the invocation
/// below with no other change and `DocumentRow::ALL.len()` moves with it, immediately.
macro_rules! document_rows {
    ($($(#[$doc:meta])* $variant:ident),+ $(,)?) => {
        /// One row of the census — a document TYPE, never a document.
        ///
        /// ★ Declared by the [`document_rows!`] macro above, which also generates [`Self::ALL`]
        /// from this same variant list — see that macro's doc comment for why.
        ...
        impl DocumentRow {
            /// Every row, in census order — GENERATED from the exact list that defines the enum
            /// itself (the [`document_rows!`] macro), so a variant cannot be added to one without
            /// the other: there is one list, not two kept in sync by hand. The exhaustive `match`es
            /// elsewhere in this file still force ARMS for a new variant; this is what additionally
            /// makes its ALL-membership automatic rather than a second thing to remember (FR-179).
            pub const ALL: &'static [DocumentRow] = &[$(DocumentRow::$variant),+];
        }
    };
}
...
mod tests {
    ...
    /// Every row round-trips through `get`/`set`. `ALL`'s completeness is no longer this test's
    /// job (FR-179): `ALL` is now GENERATED by the [`document_rows!`] macro from the same list that
    /// declares the enum, so a variant cannot exist in one without the other — an exhaustive `match`
    /// alone never proved that (it forces ARMS for a new variant, not membership in a separately
    /// typed array). The `20` below is a documentation count, not a completeness gate.
    #[test]
