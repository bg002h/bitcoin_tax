...
pub fn port_status(prior_tag: &str, new_tag: &str) -> Result<String, String> {
    ...
    for form in emitting_surface() {
        ...
        match compute(&a, &b) {
            ...
            Err(_) => {
                let prior = if pdf_for(&a).is_some() {
                    format!("`{a}`")
                } else {
                    "**NO PRIOR SIDE**".to_string()
                };
                let new = if pdf_for(&b).is_some() {
                    format!("`{b}`")
                } else {
                    "**NO DRAFT**".to_string()
                };
                let cell = if pdf_for(&b).is_none() {
                    "**NO DRAFT**"
                } else {
                    "**NO PRIOR SIDE**"
                };
    ...
    out.push_str("| form | common | added | removed | lines that moved | shape |\n|---|---|---|---|---|---|\n");
    for r in &numeric {
        out.push_str(r);
        out.push('\n');
    }
...
mod tests {
    ...
    fn port_status_prints_the_committed_work_list() {
        type Cells = Option<(usize, usize, usize, Option<usize>)>;
        let printed = super::port_status("2025", "2026-DRAFT").unwrap();
        let doc = std::fs::read_to_string(
            crate::form_geometry::repo_root().join("design/TY2026_WORK_LIST.md"),
        )
        .unwrap();
        let rows = |text: &str| -> std::collections::BTreeMap<String, Cells> {
            text.lines()
                .filter_map(parse_work_list_row)
                .map(|(form, cells, _, _)| (form, cells))
                .collect()
        };
        ...
        for (form, cells) in &p {
            assert_eq!(
                d.get(form),
                Some(cells),
                "{form}: printer says {cells:?}, the committed work list says {:?}",
                d.get(form)
            );
        }
        // the plant: a printer that dropped a stem would fail the surface equality above; prove the
        // comparison can fail by removing one row from the printed text
        let dropped: String = printed
            .lines()
            .filter(|l| !l.contains("| `f6251` |"))
            .collect::<Vec<_>>()
            .join("\n");
        assert_ne!(
            rows(&dropped).len(),
            p.len(),
            "dropping a row must change the parsed set"
        );
    }
