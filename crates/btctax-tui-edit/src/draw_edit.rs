...
/// The Approach-B experimental notice banner's text, rendered by `draw_browse` (the only screen this
/// crate still renders it on) — a SINGLE call site for `NOTICE.one_line()`, so nothing in this crate can
/// independently hand-paraphrase it and drift. ★ fix round 1 Important: MUST call `NOTICE.one_line()`,
/// never a hand-typed literal — a hand-copy here is also byte-independent of `btctax-tui`'s own banner,
/// so the two crates' banners could reword differently with nothing red.
...
fn draw_browse(frame: &mut Frame, app: &mut EditorApp) {
    ...
    let chunks = Layout::default()
        ...
        .split(area);
    // Index bookkeeping: each conditional row above shifts every index after it down by one.
    ...
    // Feature keys (c/o/r/f/v/S/d/L/u/m/i/z/e/a/A/b/B/C/V/I/O/P/T) live in the `?` KEYMAP overlay
    // (`help_overlay_lines`), not here — this footer is the NAVIGATION hint and already overflows an
    // 80/120-col terminal (its tail clips mid-line).
...
/// Any NEW `KeyCode::Char(_)` arm in the Browse match must gain a line here, or that test reds.
...
mod tests {
    ...
    /// Approach-B experimental disclosure (`design/approach-b-experimental-notice`): a Browse render
    /// (no pseudo default) with an injectable `events` list. Returns the raw `Buffer` so a test can
    /// assert on row order/shift.
    ...
    /// Browse only REFLECTS Approach-B (the filer's RETURN depends on it), so it stays correctly gated
    /// on `uses_approach_b` — an empty vault shows no banner (`browse_experimental_banner_appears_
    ...
    fn kat_keymap_overlay_lists_every_browse_char_binding() {
        ...
        // "KEEP IN SYNC with KEYMAP overlay" comment alone did not hold when this test was first added
        // (Browse `w`, the since-deleted Defensive Filing Wizard's entry point, shipped absent from the
        // overlay, leaving the feature undiscoverable in-product). This test is the mechanical guard.
