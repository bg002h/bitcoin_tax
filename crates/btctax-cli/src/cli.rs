...
pub enum Command {
    ...
    Report {
        ...
        /// §4 R3-M6: persist this year's computed charitable, QBI (business-loss and REIT/PTP)
        /// and §1212(b) capital-loss carryover-OUTs as next year's carryover-INs (a full-return
        /// `--tax-year` ReturnInputs year only). `report` is otherwise read-only; this flag opts
        /// into the vault write. A carryover you typed in yourself needs `--force` to overwrite.
        #[arg(long, default_value_t = false)]
    ...
    ///     revision it is Box C / Box F ("not reported on a 1099-B"). Never the wrong pair
    ///     for the year. More rows than a part's grid holds (11 in 2025, 14 in 2024) paginate
    ...
    ///     "k Digital assets" on the Rev. 12-2023/2025 forms (2024/2025). Leaves every OTHER party's
    ...
    ///     capital-gain line (line 7a in 2025 / line 7 in 2024, when Schedule D is
    ///     active and line 16 ≥ 0; active-and-zero → "-0-"; a net loss leaves it blank — the §1211
    ///     line-21 cap is yours) and the Digital-Asset question (YES iff any disposal,
    ///     income, gift, or donation; never a "No"). 7b checkboxes are untouched.
    ///
    ...
    ExportIrsPdf {
        ...
        /// TY2017's package was dropped in 2026, though `report --tax-year 2017` still computes the
        /// crypto delta). TY2024 is the pre-1099-DA revision: Bitcoin is filed under Box C/F (not
        /// Box I/L).
        #[arg(long)]
        tax_year: i32,
...
pub enum IncomeCmd {
    ...
    /// Project this year's return onto the household description the two independent tax engines
    /// take, and print it (JSON). Run it before you export.
    ///
    ...
    /// profit, unemployment, the four Schedule A components, which deduction line 12 claimed, the
    /// HSA deduction, and your dependents with their ages, the credit box each one lands in,
    /// blindness and your own age.
    ///
    /// WHAT IS CHECKED IS THE DESCRIPTION, NOT THE PACKET. Everything the engines are compared
    /// against — including btctax's own column — is computed from the description below, so a
    /// figure the description cannot carry is missing from every column and produces silence rather
    /// than a disagreement. Read `not_carried` before you read the verdicts.
    ///
    ...
    ///
    /// And it prints `refused` when btctax will not COMPUTE this return — the same screens
    /// `btctax report` runs, on your own inputs. The row still prints so you can see what the
    /// engines would be told, but `check_return.py` exits 2 on it: a return that does not file has
