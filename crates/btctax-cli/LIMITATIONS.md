...
## Authoring a full return

**The form is the primary authoring path.** The `btctax-tui-edit` "tax inputs" mode — press `T` on the
Browse screen — is the primary way to author a full-return `ReturnInputs`: an interactive, section-by-section
form with live per-field validation, autosave to an encrypted draft, and a commit (`s`) that screens the
return (`screen_inputs`) and writes it **only** if it is clean — a refusal jumps you to the offending field
rather than writing a bad return. `btctax income import` (loading a hand-filled TOML) — with `income show` /
`clear` / `answer` — remains the file-based path; it is complementary, not deprecated. (There is no
`income template` command: TOML files are hand-authored or scripted, never generated by btctax.)

**What the form cannot see at entry.** The commit-screen (`screen_inputs`) is the **same gate** the resolver
uses, so a return the form lets you commit is never worse than what `report` computes from it — but two blind
spots are worth naming:

- **The v1 form covers only the common subset.** Header/PII, W-2s (including box 12), Schedule A (including
  charitable), dependents, **Payments**, declarations, and the skippable optional sections. The **deferred
  sections — Schedule C, QBI, 1099-INT/DIV/G, and the capital-loss / charitable / QBI carryforwards — are not
  yet authorable in the form.** Enter those via `btctax income import` (TOML) until a later phase adds them;
  the form does not silently drop them, it simply does not offer them yet.
- **Some checks fire only at `report`, not at commit.** The advisories — FBAR/FinCEN, charitable donee class,
  the qualified-appraisal trigger, blindness not declared, the sales-tax election not asked, and the
  mixed-use mortgage allocation (see "Advisories the report will show you" below) — surface when you run
  `report`, not on the commit screen. A form-clean return can still have report-time advisories waiting for
  you. The form commits a return that is **screen-clean**, not one that is **advisory-free**.

The form removes the fiddly-TOML pain for the common case; it does not expand what v1 computes.
...
|---|---|---|
| **Child Tax Credit / Credit for Other Dependents** (Schedule 8812) | Up to $2,000 per qualifying child; $500 per other dependent — **unless §24(b) has already phased it out at your income**, in which case the credit is $0 for any composition of dependents. **1040 line 19 says which case you are in:** `0` only where the phase-out is provable, and otherwise **BLANK** — btctax will not swear to a credit it never worked out. | **Your report's advisory says the same thing in words — read it.** If it says the credit is *NOT AVAILABLE TO YOU*, line 19's $0 is the **correct** figure (Schedule 8812 line 12 answers "No", which instructs `-0-`) and there is no Schedule 8812 for you to file. Otherwise line 19 is blank and it is **yours to fill in**: file Schedule 8812 yourself and carry its line 14 there. Your tax is **overstated** until you do, by up to that amount. |
...
| **Direct deposit** of a refund (1040 lines 35b–35d) | Faster refund; a paper check can no longer be relied on. | **btctax fills this block now** — enter your routing and account numbers and the type of account (the tax-inputs editor, or `btctax income import`) and lines 35b–35d are printed. If you give none, they stay blank and the report says so. Since October 2025 the IRS generally stops issuing paper checks for federal disbursements, including refunds, unless an exception applies — so a mailed check can no longer be relied on either. |
...
- **A Form 1099-B carrying ANY adjustment — the stock ceiling, and it is permanent.** btctax takes your brokerage 1099-B as the **Schedule D line 1a/8a totals** — four numbers, no transaction list. That shortcut is the form's own: lines 1a/8a are for *"transactions reported on Form 1099-B for which basis was reported to the IRS and for which you have no adjustments."* btctax asks you to confirm **both** limbs, and **unanswered and "no" both refuse.** If either is untrue — a **wash sale** (box 1g), **accrued market discount** (box 1f), a **disallowed loss** (box 7), or a **noncovered security** (box 5) — those sales belong on Form 8949 one row at a time with an adjustment code, and btctax refuses rather than file totals on a line that cannot carry them.
  - **This is a deliberate ceiling, not a gap awaiting a fix.** btctax writes Form 8949 rows from your own **bitcoin** records only, and holds no per-sale record of a broker's transactions to itemize. If your 1099-B carries an adjustment, this program cannot file your return — use a preparer.
