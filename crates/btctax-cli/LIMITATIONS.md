...
- **Taxable income of $0 with a capital-loss carryforward you brought IN** — refuses (`TaxableIncomeNonPositiveWithCarryforward`). ★ The reason is narrower than it used to be, and worth stating exactly, because the two sides of this edge now behave differently:
  ...
  - **The carryover-out is not rolled forward for you.** `btctax report --write-carryover` stamps three carryovers into next year's inputs — the **charitable** one and **both** QBI carryforwards (qualified-business, Form 8995 line 16, and REIT/PTP, line 17); the capital-loss carryover-out is **not** among them. Read it off this year's report and enter it next year with `btctax income import`.
