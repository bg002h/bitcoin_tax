...
- **Taxable income of $0 with a capital-loss carryforward you brought IN** — refuses (`TaxableIncomeNonPositiveWithCarryforward`). ★ The reason is narrower than it used to be, and worth stating exactly, because the two sides of this edge now behave differently:
  ...
  - **Carryforward IN — still refuses**, and not because the worksheet is missing. It is *filing* this edge that has never been validated, and letting it through would widen the filing surface on a return you sign under penalties of perjury. The refusal is now mechanically liftable, which is exactly why lifting it is a deliberate decision rather than a side effect.
  - **The carryover-out is not rolled forward for you.** `btctax report --write-carryover` stamps the **charitable** and **QBI-REIT/PTP** carryovers into next year's inputs; the capital-loss carryover-out is **not** among them. Read it off this year's report and enter it next year with `btctax income import`.
  - (A refund-only filer with *no* carryforward is fine: tax = $0, withholding refunded.)
