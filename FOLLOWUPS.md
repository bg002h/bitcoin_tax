...
- **FR-50 — `forms port-status <year>`** enumerates from the emitting surface (`Stem` × year) and
  ...
  **UNWITNESSED** (FR-58), and `the_committed_work_list_matches_form_delta_at_head` already enumerates
  ...
  `port-status` was to fix. ✅ **(b) DONE 2026-09-06:** `xtask port-status <prior-tag> <new-tag>` prints both work-list tables from
  the emitting surface (`emitting_surface()` — every `irs_stem` any bundled map declares); the doc is
  regenerated by pasting it and `port_status_prints_the_committed_work_list` holds the two together. FR-50
  is closed.
...
- **FR-59 — `Payments.extension_payment: Usd` prints a sworn `0` on Schedule 3 line 10 when no
...
- ✅ **CLOSED 2026-09-13 BY OWNER RULING — "Take fable's call."** A Fable consult (`design/agent-reports/ADVICE-fable-b1b-doctrine.md`, 168 lines) was asked which of three shapes this was, and called **option 2: amend B1a, not a new B1b, and not "already implied"**. Applied to `design/HARNESS.md`: B1a is now *"the FIXTURE and the PLANT are the other half of the checker"*, its fixture body kept **verbatim**, with the plant half appended. ★ **Why not a sibling rule:** `HARNESS.md` classifies by **cure-shape, not symptom** (`:41`), and both halves have the same cure — derive from the measured set, or declare a premise with a named boundary. A B1b would be the r1 excuse-list move, and A5's substring-only mutation is already a candidate "B1c". ★ **Why not "already implied":** B1a's question (*"what makes the subject present?"*) **answers cleanly** for the old plant — the form *was* present — so B1a as written passed the case. The amendment adds the second question: *"what makes this plant a defect rather than a fact — and would it still be one on the day the set is complete?"* ★★ **It corrected the coordinator's reasoning**, which had called the recurrence the strongest argument: the first rescue was a *patch*, and patches recur, so that proves nothing. It is evidence because **correct, unrelated work broke it**, and because in a repo with a port machine an absence is *by definition the fact about to change*. **The completeness argument is the strong one**; the recurrence is it observed in the wild. ★ And it caught a false fact in this very entry — "nine days" was **six** (A5 dates the rescue 2026-09-06) — with the rule *pin the date, not the interval*, which proved itself when the interval became seven overnight. Cost, stated: one extra sentence per kill test, and plant-by-removal is usually **cheaper** (A5's went 1 → 177 plants for the same code); oracle injection is the only real expense, one-time per external-state checker. The hidden risk it names is over-application — *a kill that kills a mock* — bounded in the text by *"inject the oracle the checker consults, never the computation the checker is."*
  Original entry:
  **FR-154 — `design/HARNESS.md` does not name the shape A5 found: the PLANT is the other half of the kill. Nit → doctrine. Owning phase: the OWNER.**
  ...
  the `f8995a` plants **had already been rescued once for this exact reason on 2026-09-06** (six days before A5 found them borrowed again) — a shape that
...
- ✅ **CLOSED 2026-09-12** (A1) — `xtask line-coverage` now walks `crates/btctax-forms/forms/*/*.map.toml`, reads each map's own `irs_stem`/`year`, and requires every caption it quotes to be printed on **that year's** extract. **No year is named in the checker.** Run output: *429 map caption(s) across 38 bundled map(s) checked against their OWN year's extract (floor 429)*. ★ Controller re-planted the kill on main: swapping §213(a)'s 7.5% floor for the pre-2017 10% figure in a REAL caption of the TY2025 Schedule A map reds two tests with *"This is the Form 6251 line-33 class — a sentence carried forward from a document nobody re-read."* ★★ A1 refuted two things: this entry's mechanism citation (`line_coverage_check.rs:1067`) belongs to **FR-135** — FR-134's pinned gate was `btctax-forms/tests/f8995a_map.rs:9-10`'s two `include_str!`s, exactly as the rehearsal originally said and the controller wrongly overrode — and **the proposed one-shape parser would have read 134 of 429 captions**, because the repo writes captions FOUR ways and `f8959`'s 2024 and 2025 maps use *different* shapes for the same 17 lines, so it would have reproduced the very blindness it was built to fix. ★ Two real transcription defects found in maps outside A1's ownership (`f1040sa/2024` line 5e abbreviates "married filing separately" to "MFS"; `f8959/2024` line 8 drops "(Form 1040)") are **pinned, not edited**, keyed to the wrong sentence so the pins go stale-and-red the day the maps are fixed — see FR-153.
  Original entry:
...
- ◐ **NOW-HALF CLOSED 2026-09-12** (A1); **remainder re-owned to the port machine.** The regression is prevented: the checker derives the bundled `(form, year)` set from the maps and **ratchets the pairs whose coverage rows quote a different year — 12 today, enumerated by the run** — so bundling `f6251/2026` reds instead of silently leaving 42 captions pointed at TY2024. `f6251--2025` and `f1040sd--2025` are correctly absent from the 12, which is the rule discriminating rather than counting. What remains is re-pointing those 12, which needs per-year coverage rows for twelve forms — the revision-aware refactor this entry names as its full fix and which the rehearsal itself placed in the port-machine phase, not NOW. **Owning phase (remainder): the port machine / step-24 widening.**
  Original entry:
...
- ✅ **CLOSED 2026-09-12** (S4) — corrected in `design/TY2026_PORT_REPORT.md` beside the 24-step table, with inline pointers on steps 1/11/12, and **independently re-verified rather than transcribed**: S4 read the `build.rs` panic and the `xtask → btctax-forms` dependency in source itself.
  Original entry:
...
- ✅ **CLOSED 2026-09-12** (A3) — and the headline is far bigger than this entry predicted. **The excuse list went from 31 pairs to 1, and NO archive was created to do it: 30 of the 31 committed `"not-yet-archived"` claims were FALSE.** Controller-verified independently before integration (30 false / 1 genuine, the survivor being exactly the `f8283--2024` A3 named). `archived_form_years()` now joins each map row (`irs_stem` + `instructions` + year) to `design/forms/MANIFEST.json`, requiring for the form **and** its instructions an entry that is IRS-final not draft, records the conventional `design/forms/extract/<stem>--<year>.txt`, and is present on disk — all committed inputs, so it runs in a worktree with zero PDFs. `AUTHORITY_NOT_YET_ARCHIVED` is deleted; the excuse set is read off each row's own `authority = "not-yet-archived: <reason>"` header and an arbitrary string is refused. ★ **Not a weakened gate:** the shrink-only human declaration still exists once, at `tests/map_rows.rs::EXCUSED`, pinning the header set in both directions — what was removed is the THIRD copy, the one carrying 30 false entries. Controller-measured on main: `cite-check` goes **7/38 → 37/38 archived, 1 excused, 0 unaccounted**. ★ A3's M2 mutation is the proof that matters: ignoring the instructions document makes the ratchet **stop complaining**, and the kill catches that — the difference between discriminating and merely passing.
  Original entry:
...
- ✅ **CLOSED 2026-09-12** (A2) — **7 → 1 hand-edits** per new `(stem, year)` in `build.rs` + `line_set.rs` + `f6251_revision.rs`, measured by planting the data half of the rehearsal's own `f8995a/2025` port on the pristine tree and again on the fixed tree, applying forced edits one at a time. `build.rs` now generates `LineSet` + `parse` + `as_str` + `ALL` from each map's `line_set` row (strict line scan, no new dependency); the generated variant set is identical to the hand-written one (38 = 38, clean diff). `line_set.rs` keeps only `Schema` and `schema()`, still `_`-free and now a `const fn`. `f6251_revision::revision()`'s match over every `LineSet` became `_ => None` **guarded by a `const _: ()` walk** over the revisions whose schema is `Form6251ObbbaMap`, so porting an unrelated stem no longer reds a Form-6251 module. ★★ **A2's refutation is the more valuable half: `LineSet::ALL`'s entry was forced by NOTHING** — with the variant, `parse`, `as_str`, `schema` and the f6251 arm in place and `ALL` omitted, the suite's failure set was identical, so a port that forgot that one line left its revision unmeasured by every gate iterating `ALL` (controller-confirmed: `ALL` is iterated by six files, four of them tests). ★ Also refuted: the per-form cost in these files was **7, not 6**, and edits 2 and 4 were misattributed to `tests/map_rows.rs`, which never mentions `LineSet` — the reds come from `supported_years_cross_product` and `field_census`.
  Original entry:
...
- ✅ **CLOSED 2026-09-12** (S1) — all 10 fill cases now run `for year in covered_years()`, where the set derives from `bundled::BUNDLED` filtered through `Form8995AMap::for_year(year).is_ok()` — **the same schema dispatch `packet.rs` uses in production**. No hardcoded year literal remains, so a future port is picked up with **zero edits to this file**, and a year whose schema diverges is *excluded* rather than assumed compatible. Measured scope today: exactly `{2024}`. ★ Discrimination preserved deliberately: every assertion embeds `TY{year}` in its failure message, and a new test (`the_covered_year_set_can_never_be_silently_empty`) proves the empty-set guard panics rather than letting a loop over nothing pass. Kill: a planted wrong cell value reds naming the year.
  Original entry:
...
- ✅ **CLOSED 2026-09-12** (S2) — chose **option 1, derive**, but from the filesystem glob `crates/btctax-forms/forms/*/*.map.toml` rather than by parsing the Rust enum, and the reason is the good one: **that glob is what `build.rs` uses to build `Stem` in the first place**, so a form in the glob without a `Stem` variant already fails the Rust build — the two cannot silently diverge and no fragile Rust parser was needed. `draft_stems()` globs, dedupes across years, and translates the two crate-stem exceptions (`schedule_d`→`f1040sd`, `schedule_se`→`f1040sse`) through a 2-entry table that **refuses loudly on any unmapped underscore**. 18 → **21** stems; `f1040v`, `f4868` and `f8889` are now actually fetched. Kill: reverting `draft_stems()` to ignore disk reds with 8 distinct messages across 4 rows.
  Original entry:
...
- ✅ **CLOSED 2026-09-12** (S4) — the runbook now states that steps 11–21 are **one atomic commit**, unbuildable in between.
  Original entry:
...
- ✅ **CLOSED 2026-09-12** (A2) — the misplaced `LineSet` doc comments are fixed as a side effect of generating the enum: the doc line for `"f8959/2024"` no longer sits above `F8889_2024`. A2 reports a generator kill (a shifted doc line reds two tests) and, honestly, that a *swap* plant was green under a set comparison and is caught only by the per-map attribution check.
  Original entry:
...
- ✅ **CLOSED 2026-09-12** (S3) — the mechanism was exactly as filed: `ensure_xtask_binary()` spawns `cargo build -p xtask` **without clearing `CARGO_TARGET_DIR`**, so under the mandated override the binary lands in the override's tree while the function's `exists()` checks — and `scripts/hooks/on-write.sh` — look at the hardcoded `<repo>/target/debug/xtask`. Fixed with one line, `.env_remove("CARGO_TARGET_DIR")`, **no skip**. Verified three ways from a clean `target/`: before, 7 failures under the override; after, **6** (all `form_delta`, gitignored PDFs); and still passing with the override unset, so the default path did not move. ★ This is why the standing "a worktree result is not comparable to main" caveat existed — and a caveat that excuses a real, fixable failure trains everyone to ignore reds. One cosmetic note: the helper is `ensure_xtask_binary`, not `ensure_xtask`.
  Original entry:
...
- ✅ **CLOSED 2026-09-12** (S4) — step 1's real output is `YEAR.toml`, now said so; verified live against `forms/2025/YEAR.toml`, which still lists `f8995a` under `[forms_absent]`.
  Original entry:
...
- ✅ **CLOSED 2026-09-12** (A3) — the `38` is gone with **no replacement number and no floor**. The expectation derives from each year's `YEAR.toml` `forms_expected`, giving three independently-authored views of one set (templates / map rows / year declarations). Vacuity is structurally impossible: `map_rows()` refuses a walk with no rows or no `f1040`. Kill M5 reds the derivation naming all 18 TY2025 rows.
  Original entry:
...
- ✅ **CLOSED 2026-09-12** (S4) — **resolved, not merely flagged.** S4 measured (grep + a parse of `schema()`) that **all 38 maps mint a fresh per-year `line_set` string**, including all 16 of 18 TY2025 maps that are constants-only by §4's own definition. So §4's *"constants-only year ⇒ same line_set"* was simply false, and the doc now states the real invariant: **`line_set` is always minted per year; constants-only vs. renumber only decides which `schema()` arm it resolves to.** Judged resolvable from the document's own later prose plus the built tree, so it was not left as an owner question — the right call, and the measurement is in the report.
  Original entry:
